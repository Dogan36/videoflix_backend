...
class MovieTasksTest(TestCase):
    def setUp(self):
        # ein kleines Dummy-Video anlegen
    ...
    # Original sichern und in tasks_module ersetzen
    ...
    def test_finalize_conversion_deletes_original_and_clears_field(self):
        # Stelle sicher, dass video_file existiert
        m = Movie.objects.get(pk=self.movie.id)
        ...
        # Feld auf None
        m.refresh_from_db()
        self.assertFalse(m.video_file)  # leerer FileField evaluiert als False